...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="DeepTune-style Transformer over raw LLVM IR for the paper-end2end-dl datasets."
    )
...
def main():
    ...
    tokenizer_path = out_dir / "tokenizer" / "wordpiece_tokenizer.json"
    ...
    if args.task in {"device-mapping", "all"}:
        ...
        print(f"Saved device-mapping results to: {results_dir / 'device_mapping_results.csv'}")
    ...
    if args.task in {"thread-coarsening", "all"}:
        ...
        print(f"Saved thread-coarsening results to: {results_dir / 'thread_coarsening_results.csv'}")
