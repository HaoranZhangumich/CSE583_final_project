...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run source/IR × transformer/LSTM experiments.")
    parser.add_argument("--repo-root", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, default=Path("./out"))
    parser.add_argument("--clang-bin", type=str, default="clang")
    parser.add_argument("--seed", type=int, default=204)

    parser.add_argument("--input-mode", choices=["ir", "source"], required=True)
    parser.add_argument("--model-type", choices=["transformer", "lstm"], required=True)
...
def main():
    ...
    # Always build IR cache first, because source mode also uses the IR-valid subset
    if args.task in {"build-ir", "train-tokenizer", "device-mapping", "thread-coarsening", "all"}:
    ...
    if args.task in {"device-mapping", "all"}:
        dm = run_device_mapping_cv(
            repo_root, ir_cache_dir, tokenizer_path, results_dir, args.seed, cfg,
            input_mode=args.input_mode, model_type=args.model_type
        )
        print(dm.groupby("platform")[["correct", "speedup_vs_default"]].mean())

    if args.task in {"thread-coarsening", "all"}:
        tc = run_thread_coarsening_cv(
            repo_root, ir_cache_dir, tokenizer_path, results_dir, cfg,
            input_mode=args.input_mode, model_type=args.model_type
        )
